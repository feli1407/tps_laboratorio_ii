...
namespace FrmLocalDeVideoJuegos
{
    public partial class FrmPrincipal : Form
    {
        ...
        private void FrmPrincipal_Load(object sender, EventArgs e)
        {
            ...
            stock = new Stock();
            stock.OnNotificarStock += NotificarStockVideoJuegos;
            stock.Iniciar();
            reloj = new Reloj();
        ...
        /// <summary>
        /// Creo un delegado de tipo Action donde le asigno la funcion NotificarStockVideoJuegos y hago que en el caso
        /// de que hayan VideoJuegos con el stock en 0 me notifique mediante un messagebox.
        /// </summary>
        private void NotificarStockVideoJuegos()
        {
            if (lblPorcentajeDeGananciaDeseado.InvokeRequired)
            {
                Action delegado = NotificarStockVideoJuegos;
                lblPorcentajeDeGananciaDeseado.Invoke(delegado);
            }
            else
            {
                int flag = 0;
                StringBuilder sb = new StringBuilder();
                foreach (var v in this.local.VideoJuegos)
                {
                    if (v.Stock == 0)
                    {
                        flag = 1;
                        sb.Append(v.Nombre);
                        sb.AppendLine(" de " + VideoJuego.TipodDeVideoJuego(v));
                    }
                }
                if (flag == 1)
                {
                    MessageBox.Show("Los VideoJuegos con stock en 0 son: \n" + sb.ToString(), "VideoJuegos Sin Stock");
                }
            }
        }
                    ...
                    foreach (VideoJuego v in local.VideoJuegos)
                    {
                        ...
                        this.dgvJuegos.CurrentRow.Cells[4].Value.ToString() == VideoJuego.TipodDeVideoJuego(v))
                        {
                            if (v.Stock > 0)
                            {
                                ...
                                {
                                    MessageBox.Show("La venta se cancelo correctamente.", "Notificación");
                                }
                            }
                            else
                            {
        ...
        /// </summary>
        private void GuardarSQLVideoJuegos()
        {
            List<JuegoPlay> juegosPlay = new List<JuegoPlay>();
            List<JuegoXbox> juegosXbox = new List<JuegoXbox>();
            List<JuegoNintendo> juegosNintendo = new List<JuegoNintendo>();
            ...
            try
            {
                ...
                MessageBox.Show("Se guardaron correctamente los datos en la base de datos.", "SQL");
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Error");
